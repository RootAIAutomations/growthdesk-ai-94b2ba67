...
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='content')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='message') then
    alter table public.message_log rename column content to message;
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='channel')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='message_type') then
    alter table public.message_log rename column channel to message_type;
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='sent_at')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='message_log' and column_name='interaction_date') then
...
-- follow_up_schedule renames
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='follow_up_schedule' and column_name='task')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='follow_up_schedule' and column_name='title') then
...
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='follow_up_schedule' and column_name='completed') then
    update public.follow_up_schedule
    set status = case when completed is true then 'Completed' when status is null then 'Pending' else status end,
        priority = coalesce(priority, 'Medium');
  else
    update public.follow_up_schedule set status = coalesce(status,'Pending'), priority = coalesce(priority,'Medium');
  end if;
end $$;
...
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='outreach_drafts' and column_name='content')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='outreach_drafts' and column_name='draft_text') then
...
set channel = case lower(coalesce(channel,'whatsapp'))
    when 'whatsapp' then 'WhatsApp' when 'email' then 'Email' when 'linkedin' then 'LinkedIn' else 'Other' end,
    status = case lower(coalesce(status,'draft'))
    when 'draft' then 'Draft' when 'reviewed' then 'Reviewed' when 'copied' then 'Copied' when 'archived' then 'Archived' else status end;

do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='outreach_drafts' and column_name='created_at') then
    update public.outreach_drafts set generated_at = coalesce(generated_at, created_at::timestamptz, now());
  else
    update public.outreach_drafts set generated_at = coalesce(generated_at, now());
  end if;
end $$;
...
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_calendar' and column_name='scheduled_date')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_calendar' and column_name='content_date') then
    alter table public.content_calendar rename column scheduled_date to content_date;
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_calendar' and column_name='title')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_calendar' and column_name='topic') then
...
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_calendar' and column_name='notes') then
    update public.content_calendar set linkedin_post = coalesce(linkedin_post, notes);
  end if;
end $$;

update public.content_calendar
set status = case lower(coalesce(status,'generated'))
    when 'planned' then 'Generated' when 'drafted' then 'Generated' when 'scheduled' then 'Saved' when 'published' then 'Published' else status end,
    generated_at = coalesce(generated_at, created_at::timestamptz, now());

-- content_library
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_library' and column_name='body')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_library' and column_name='content') then
    alter table public.content_library rename column body to content;
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_library' and column_name='content_type')
     and not exists (select 1 from information_schema.columns where table_schema='public' and table_name='content_library' and column_name='platform') then
